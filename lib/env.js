...
function Env(input, output, settings, cli) {
  ...
  if(this.app.isnt_packaged) {
    ...
    if(process.env.METEOR_VERSION_OVERRIDE) {
      console.log("Overriding Meteor version to " + process.env.METEOR_VERSION_OVERRIDE);
      meteor_symlink = meteor_symlink.replace(/[0-9]+\.[0-9]+\.[0-9]+/m, process.env.METEOR_VERSION_OVERRIDE);
      console.log("Meteor path: " + meteor_symlink);
    }
